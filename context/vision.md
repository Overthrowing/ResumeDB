...
3. For each generated resume, store an internal json/yaml of the data before it gets converted to typst/latex via a template. The agent can modify the typst/latex directly, but ideally the autoconvert is enough and the agent only touches it when there are space constraint issues.
4. Auto-tailor the resume to the specific opening and specific company.
5. Optimize keywords but go beyond just this by modifying user-input database entries as needed to fit the job role and space limitations.
6. Each application -> new project with bucketed context containing app details + chat histories for resume editing.
7. Nudge user for details when needed (e.g. quantitative metrics).
8. Focus on quantitative metrics?
9. LLM-written cover letter/short answer questions (these can be requested in the chat).
10. Run resume auditing tools to see how your resume compares -> auto-optimization loop (questionable since this can lead to local minima).
11. Autoscan and put job openings into a central spreadsheet.
