...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Cormorant Garamond for all
  display + Lora body + JetBrains Mono for the index/code voice on a warm-navy code surface,
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Cormorant Garamond figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Cormorant Garamond italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Cormorant Garamond italic, small Cormorant Garamond uppercase cite beneath."
  section-rule:
...
Three editorial voices, each in its own face: **Cormorant Garamond** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Cormorant Garamond** carries body, leads, card titles, buttons, and
...
- **Cormorant Garamond** (sentence case, negative-tracked) for all display; **Cormorant Garamond** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one Cormorant Garamond display moment dominates at 3–6× its neighbor.
...
- **Type** — Cormorant Garamond sentence-case display (negative-tracked); Cormorant Garamond 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Cormorant Garamond `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Cormorant Garamond `headline` 4.6cqw
...
- **Cormorant Garamond display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Cormorant Garamond body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Cormorant Garamond figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Cormorant Garamond italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Cormorant Garamond · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Cormorant Garamond `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Cormorant Garamond 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Cormorant Garamond line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Cormorant Garamond `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a Cormorant Garamond `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: Cormorant Garamond italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a Cormorant Garamond **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small Cormorant Garamond uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** Cormorant Garamond italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short Cormorant Garamond sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **Cormorant Garamond, sentence case**, negative-tracked; **Cormorant Garamond 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Cormorant Garamond figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Cormorant Garamond display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Cormorant Garamond display moment dominates.
...
- **Type** — Cormorant Garamond sentence-case display (negative-tracked); Cormorant Garamond 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
